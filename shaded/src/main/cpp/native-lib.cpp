...
extern "C"
JNIEXPORT void JNICALL Java_com_feresr_shaded_PingPongRenderer_loadIntoOpenGl(JNIEnv *jenv, jclass thiz, jint texture) {
    if (current.pixels == NULL) return;
    glBindTexture(GL_TEXTURE_2D, texture);
    glTexImage2D(GL_TEXTURE_2D, 0, GL_RGBA, current.w, current.h, 0, GL_RGBA, GL_UNSIGNED_BYTE, current.pixels);
}

// Stores the bitmap into native memory to avoid OOMs on the JVM
extern "C"
JNIEXPORT void JNICALL Java_com_feresr_shaded_PingPongRenderer_storeBitmap(JNIEnv *jenv, jclass thiz, jobject src) {
...
extern "C"
JNIEXPORT void JNICALL Java_com_feresr_shaded_PingPongRenderer_freeBitmap(JNIEnv *jenv, jclass thiz) {
    delete[] current.pixels;
    current.pixels = NULL;
}

extern "C"
JNIEXPORT jboolean JNICALL Java_com_feresr_shaded_PingPongRenderer_isBitmapStored(JNIEnv *jenv, jclass thiz) {
    return current.pixels != NULL;
}

extern "C"
JNIEXPORT jint JNICALL Java_com_feresr_shaded_PingPongRenderer_getBitmapWidth(JNIEnv *jenv, jclass thiz) {
    return current.w;
}

extern "C"
JNIEXPORT jint JNICALL Java_com_feresr_shaded_PingPongRenderer_getBitmapHeight(JNIEnv *jenv, jclass thiz) {
    return current.h;
}
